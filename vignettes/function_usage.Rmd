...
示例数据存放在包根目录的 data/ 下，可通过 load() 加载，加载后，环境中应包含以下对象：
tasks、self_resource、outsourced_resource、resource_windows、resource_joint_cost、resource_mapping
...
功能：判断解 a 是否在多目标最小化意义下支配解b。

支配定义：对所有目标：a[i] <= b[i]，至少存在一个目标：a[i] < b[i]
...
#### 3.3 non_dominated_sort()

功能：对粒子群进行非支配排序，返回每个粒子的 Pareto 等级（rank）。
rank = 1 表示第一前沿（最优）
...
功能：计算同一 Pareto 前沿内粒子的 拥挤度距离， 用于保持解的多样性。
...
```{r, fig.cap="图片描述1", out.width="50%", fig.align="center"}
knitr::include_graphics("../inst/example/AAS-CN-2019-0044-5.jpg")
```

#### 3.5 update_particles()

功能：
执行一次 PSO 粒子更新，包括：速度更新、位置更新、个体最优（pbest）更新、与外部档案的比较。
```{r,eval=FALSE}
...
功能：维护外部 Pareto、 精英档案：合并新解、非支配筛选、基于拥挤度的容量控制
```{r, eval=FALSE}
#' 管理外部存档（限制大小，使用拥挤距离）
#'
...
```
返回值：pareto_front：Pareto 最优解列表

#### 3.9 plot_gantt_chart()

功能：将某一 Pareto 解对应的调度方案绘制为甘特图。
...
```{r, fig.cap="图片描述", out.width="50%", fig.align="center"}
